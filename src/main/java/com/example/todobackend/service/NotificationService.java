...
import com.example.todobackend.event.NotificationDeleted;
import com.example.todobackend.event.NotificationSaved;
import com.example.todobackend.entity.Notification;
import com.example.todobackend.repository.NotificationRepository;
...
public class NotificationService {
    ...
    public Event toEvent(final ChangeStreamEvent<Notification> changeStreamEvent){
        ...
        switch (changeStreamEvent.getOperationType()){
            case DELETE:
                event = new NotificationDeleted().setId(((BsonObjectId) changeStreamEvent.getRaw().getDocumentKey().get("_id")).getValue().toString());
                break;
            case INSERT:
            case UPDATE:
            case REPLACE:
